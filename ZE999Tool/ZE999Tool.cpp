	...
	do {
		...
		if (cmd == "bin-unpack") {
			...
			if (!tool.Unpack(ToAbsolutePath(argv[2]), ToAbsolutePath(argv[3])))
				break;
			printf("Unpacked %llu Dlgs, %llu Names, %llu Fonts.", tool.dlgs.size(), tool.names.size(), tool.fonts.size());
		}
			...
			if (!tool.Repack(ToAbsolutePath(argv[2]), ToAbsolutePath(argv[3])))
				break;
			printf("Repacked %llu Dlgs, %llu Names, %llu Fonts.", tool.dlgs.size(), tool.names.size(), tool.fonts.size());
		}
			...
			if (!tool.CopyValid(ToAbsolutePath(argv[2]), ToAbsolutePath(argv[3])))
				break;
			printf("Copied %llu Dlgs, %llu Names, %llu Fonts.", tool.dlgs.size(), tool.names.size(), tool.fonts.size());
		}
			...
			if (!tool.Patch(ToAbsolutePath(argv[2]), ToAbsolutePath(argv[3]), argv[4], ToAbsolutePath(argv[5])))
				break;
			printf("Patched %llu Dlgs, %llu Names, %llu Fonts.", tool.patch_dlgs.size(), tool.patch_names.size(), tool.patch_fonts.size());
		}
		else {
			break;
		}
