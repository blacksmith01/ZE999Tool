...
public:
	bool Unpack(fs::path src_path, fs::path dst_dir_path)
	{
		...
		//for (auto& sir : locations) {
		//	SirXmlWriter::Write(*sir, fs::path(dst_dir_path).append(sir->filename + ".location.xml"));
		//}
		for (auto& sir : msgs) {
		...
		for (auto& sir : fcharts) {
			SirXmlWriter::Write(*sir, fs::path(dst_dir_path).append(sir->filename + ".fchart.xml"));
		}
	...
	bool CopyValid(fs::path org_dir_path, fs::path dst_dir_path)
	{
		...
		//		for (auto& sir : locations) filenames.push_back(sir->filename);
	...
	bool GeneratePatchFontData(fs::path org_dir_path, fs::path patch_dir_path, fs::path bmf_default_path, fs::path bmf_border_path, fs::path dst_dir_path)
	{
		...
		std::set<wchar_t> w_keycodes;
		RetrievePatchChars(scope_min, scope_max, w_keycodes);
		RetrieveExPatchChars(fs::path(patch_dir_path).append("ze1.exe.xml"), scope_min, scope_max, w_keycodes);
		for (auto& w : w_keycodes) {
			keycode_alloc.Alloc();
			auto fn = std::make_shared<SirFont::Node>();
	...
	void RetrievePatchChars(wchar_t scope_min, wchar_t scope_max, std::set<wchar_t>& w_keycodes)
	{
		...
		//for (auto& ps : patch_locations) {
		//	auto d = FindSirPtr(locations, ps->filename);
		//	if (d == nullptr) {
		//		continue;
		//	}

		//	for (auto& pn : ps->nodes) {
		//		auto n = stdext::FindPtr<SirLocation::Node>(d->nodes, [&pn](auto& arg) {
		//			return arg->map_key == pn->map_key;
		//		});
		//		if (n == nullptr) {
		//			continue;
		//		}
		...
		//		for (auto& pitem : pn->items) {
		//			if (auto item = stdext::FindPtr<SirLocation::Node::Item>(n->items, [&pitem](std::shared_ptr<SirLocation::Node::Item> f) {
		//				return f->key == pitem->key;
		//			})) {
		...
		for (auto& ps : patch_fcharts) {
			auto d = FindSirPtr(fcharts, ps->filename);
			if (d == nullptr) {
				continue;
			}

			for (auto& pn : ps->nodes) {
				auto n = stdext::FindPtr<SirFChart::Node>(d->nodes, [&pn](auto& arg) {
					return arg->id1 == pn->id1 && arg->id2 == pn->id2;
				});
				if (n == nullptr) {
					continue;
				}
				...
				if (pn->items.size() == item_size)
				{
					for (std::size_t i = 0; i < item_size; i++)
					{
						...
						if (pitem->text != item->text) {
							auto modlen = pitem->patch_text.length();
							for (std::size_t j = 0; j < modlen; j++) {
								if (pitem->patch_text[j] >= scope_min && pitem->patch_text[j] <= scope_max) {
									w_keycodes.insert(pitem->patch_text[j]);
								}
							}
						}
					}
	...
	bool Patch(fs::path org_dir_path, fs::path patch_dir_path, std::string option, fs::path dst_dir_path)
	{
		...
		std::vector<SirLocation*> patched_locations;
		...
		//for (auto& pl : patch_locations) {
		//	auto d = FindSirPtr(locations, pl->filename);
		//	if (d == nullptr) {
		//		continue;
		//	}

		//	for (auto& pn : pl->nodes) {
		//		auto n = stdext::FindPtr<SirLocation::Node>(d->nodes, [&pn](auto& elm) {
		//			return elm->map_key == pn->map_key;
		//		});
		//		if (n == nullptr) {
		//			continue;
		//		}
		...
		//					item->text = wcs_to_mbs(fwtext, "");
		//				}
		//			}
		//		}
		//	}
		//	patched_locations.push_back(d);
		//}
		...
		for (auto sir : patched_locations) {
			SirWriter::Write(*sir, fs::path(dst_dir_path).append(sir->filename + ".sir"));
		}
	...
	bool ExePatch(const fs::path& org_dir_path, const fs::path& patch_dir_path, const fs::path& exe_file_path, const fs::path& dst_dir_path)
	{
		...
		for (auto i : fs::recursive_directory_iterator{ patch_dir_path }) {
			if (i.is_regular_file()) {
				if (StrCmpEndWith(i.path().filename().string(), ".font.xml"))
				{
					ReadXml(i.path(), true);
				}
			}
	...
	bool ReadSirFile(fs::path file_path)
	{
		...
		try {
			...
			//else if (SirReader::IsValidLocation(rbuffer)) {
			//	locations.push_back(SirReader::ReadLocation(file_path.stem().stem().string(), rbuffer));
			//}
				...
				fcharts.push_back(SirReader::ReadFChart(file_path.stem().stem().string(), rbuffer));
			}
			else {
				return false;
			}
	...
	void ReadXml(fs::path file_path, int is_patch = false)
	{
		...
		//else if (StrCmpEndWith(filename, ".location.xml")) {
		//	(is_patch ? patch_locations : locations).push_back(SirXmlReader::ReadLocation(file_path));
		//}
		...
		else if (StrCmpEndWith(filename, ".fchart.xml")) {
	...
	//std::vector<std::shared_ptr<SirLocation>> locations;
	...
	//std::vector<std::shared_ptr<SirLocation>> patch_locations;
